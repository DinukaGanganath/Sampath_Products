...
public interface SalesrepDao extends JpaRepository<Salesrep, Integer> {
    ...
    // get Last Added Salesrep
    @Query(value = "select * from sampathproducts.salesrep_details where salesrep_id=(select max(salesrep_id) from sampathproducts.salesrep_details);", nativeQuery = true)
    public List<Salesrep> getLatestSalesrep();
