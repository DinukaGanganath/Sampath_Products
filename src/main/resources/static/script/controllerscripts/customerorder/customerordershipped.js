...
// table value load
function loadTable(){
    fetch("/customerorder/findall/shipped")
...
function processOrder(ele,order){

    viewOrder(order);

    var storeok = 1;
    var notification = "Following Products are not available. \n\n";
  
        for(var orderProduct of order.customerOrderHasProductList){
            if(orderProduct.product_id.product_has<orderProduct.quantity){

                notification += `\nProduct --> ${orderProduct.product_id.producttype_id.producttype_name.replaceAll("_", " ")} ${orderProduct.product_id.productsize_id.productsize_name.replaceAll("_", " ")}\nAvailable --> ${orderProduct.product_id.product_has} Packets\nNeed --> ${orderProduct.quantity} Packets\n..............................................`;
